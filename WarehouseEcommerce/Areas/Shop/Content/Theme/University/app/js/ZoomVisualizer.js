...
(function($) {
  ...
  $.fn.ZoomVisualizer = function(method) {
    ...
    if (methods[method]) {
      //caso exista um método, esse método é chamado
      return methods[method].apply(
        this,
        Array.prototype.slice.call(arguments, 1)
      );
    } else if (typeof method === "object" || !method) {
      //caso não exista um método ou seja apenas passado o objeto
    ...
    function initialize($this) {
      ...
      $(".content", wrapper_geral).css({
        top: _data.positionZoom.top,
        left: _data.positionZoom.left
      });
      ...
      hDragMove =
        _data.sliderOrientation == "horizontal"
          ? _box.width() - _drag.width()
          : _box.height() - _drag.height();
      ...
      _drag.draggable({
        ...
        start: function(event, ui) {
          _box
            ...
            .parent()
            .mouseleave(function() {
              $(".ui-draggable-dragging", wrapper_geral)
    ...
    function checkBackgroundMouseDown(evt) {
      if (
        $(event.target).hasClass("wrapper") ||
        $(event.target).hasClass("lightbox-ofertas-bg")
      ) {
        closeVisualizer();
      }
    }
    ...
    function moveImage() {
      ...
      $(".wrapper", wrapper_geral).mousemove(function(e) {
        ...
        if (_thumb.height() > heightMask) {
          if (dgmY < -0.05 || dgmY > 0.05) _thumb.css({ top: ctposY + dgmY });
        }

        if (_thumb.width() > widthMask) {
          if (dgmX < -0.05 || dgmX > 0.05) _thumb.css({ left: ctposX + dgmX });
        }
    ...
    function initListaThumbs() {
      ...
      if ($(wrapper_geral.find(".footer .item-zoom-image.ativo")).size() > 0) {
        index_ativo = $(
          wrapper_geral.find(".footer .item-zoom-image.ativo")
        ).attr("alt");
      }

      $(wrapper_geral.find(".footer .item-zoom-image")).each(function(
        index,
        element
      ) {
    ...
    function addLoader() {
      if (_data.loader != "") {
        ...
        var _loader = $(
          "<div class='loader_overlay'><img src='" + _data.loader + "'></div>"
        );
    ...
    function resizeHandler() {
      ...
      var _width =
        $(window).width() -
        (_data.positionZoom.left + _data.positionZoom.right);
      var _height =
        $(window).height() -
        (_data.positionZoom.top + _data.positionZoom.bottom) -
        wrapper_geral.find(".footer").height() -
        $("#aba-lista").height();
    ...
    function reposImage() {
      ...
      var posDrag =
        _data.sliderOrientation == "horizontal"
          ? parseFloat(getSize(_drag, "left"))
          : parseFloat(getSize(_drag, "top"));
      var _newWid;
      var _newHei;
      var _posMaxDrag =
        _data.sliderOrientation == "horizontal"
      ...
      if (_wrapperResize) {
        if (!firstLoad) {
          //checa se foi a primeira vez que a imagem foi carregada

          firstLoad = true;

          scaleResize = Math.min(
            _wrapper.width() / _originalWidth,
            _wrapper.height() / _originalHeight
          );
          ...
          if (_data.resizeInitial) {
            //redimensiona imagem para ela caber no frame de visualização
          ...
          var _newHeightImg =
            ((_originalHeight - _wrapper.height()) / _posMaxDrag) * posDrag +
            _wrapper.height();
