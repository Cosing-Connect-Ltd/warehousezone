...
namespace WMS.Controllers.WebAPI
{
    ...
    public class ApiPallettrackingSyncController : BaseApiController
    {
        ...
        {
            var orders = OrderService.GetAllOrdersByTenantId(tenantId: 1).Where(u => (string.IsNullOrEmpty(orderNumber) || u.OrderNumber == orderNumber) && u.InventoryTransactionTypeId == InventoryTransactionTypeEnum.SalesOrder && u.OrderStatusID==OrderStatusEnum.Active).OrderByDescending(u => u.OrderID).Take(10).Select(u => new
            {
                u.OrderID,
                u.OrderNumber,
